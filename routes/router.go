...
func SetupBooksRoute(app *fiber.App) {
	...
	app.Get("/books/get/:id_book", controllers.GetBookByID)			// Mengatur route untuk mengambil data buku berdasarkan ID
	app.Post("/books/create", controllers.CreateBook) 				// Mengatur route untuk menambahkan data buku
	app.Put("/books/update/:id_book", controllers.UpdateBook) 		// Mengatur route untuk memperbarui data buku
	app.Delete("/books/delete/:id_book", controllers.DeleteBook) 	// Mengatur route untuk menghapus data buku
